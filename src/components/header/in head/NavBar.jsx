...
export default function NavBar(props) {
  ...
  return (
    <nav className="px-2 bg-white border-gray-200 " >
      <div className={`flex container  flex-wrap items-center justify-between mx-auto py-2 border-y md:border-0`}
      >
        ...
        <a href="#" className=" md:border-r text-gray bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium  text-sm px-4 lg:px-5 py-2 lg:py-2.5 mr-2  focus:outline-none ">
         ...
         {/*start sideBar*/}
         <div className={`${!props.nav &&'hidden'} md:hidden flex fixed top-0 left-0  w-screen h-screen bg-[rgba(0,0,0,.5)] `} 
          >
            <button id='ay-navBtn' 
                ...
                <h3 className='font-bold mt-2'>
                ...
                </h3>
                  {
                    Object.entries(props[0].children).map(sub=>{
                      ...
                      return(
                        <li className='my-4 '  >
                          <div className='flex justify-between hover:text-red-600' onClick={(e)=>showLiHandler(e)}>
                          <span>{sub[1].title}</span>
                          <svg className="" width='20px' aria-hidden="true" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd"></path></svg>
                          </div>
                          <ul className=' hidden bg-gray-50 rounded px-2 '>
                            <a href="#" className=' text-sm text-gray-500 font-semibold '>
                              <li className='my-3'>
                              همه موارد این دسته
                              </li>
                            </a>
                            {
                              Object.entries(sub[1].children).map(subSub=>{
                                return(
                                  <li >
                                    <div className='flex justify-between hover:text-red-600 my-2' onClick={(e)=>showLiHandler(e)}>
                                          ...
                                          return(
                                            <li>
                                              <div className='flex justify-between hover:text-red-600'>
                                                <span className='my-2 text-sm font-semibold'>{liItem}</span>
                                              </div>
                                            </li>
                                          )
                                        })
         ...
         <ul className="flex  p-4 m-4 mt-0 text-sm rounded-lg font-medium bg-white">
          
          {
            Object.entries(props).map(list=>{
              return(
                <li className='transition-transform'>
                  <a className="flex items-left font-medium text-gray-700 hover:text-red-700 p-0  hover:border-b-2 hover:border-b-red-700 ml-4">
                    ...
                    list[1].hasChild&& (
                      ...
                      className="hidden md:absolute w-11/12 relative max-h-[80vh]  font-normal bg-white  shadow overflow-y-auto    ">
                        ...
                        <div className="flex flex-col max-h-[90vh]">
                          ...
                          <a href="#" className=' my-5 text-xl  '> تمامی محصولات {mainSubItem.title}</a>
                          <div className="flex">
                            <div className=' flex flex-col flex-wrap max-h-[60vh] '>
                            {
                              mainSubItem.children.map(item=>{
                                return(
                                <>
                                    <h3 className="mb-6 text-xs font-semibold mx-3 px-2 border-r-2 border-red-600 hover:text-red-700  ">{item.title} </h3>
                                    
                                      {
                                        item.children.map(des=>{
                                          return(
                                            <li className="mb-4 mx-3 hover:text-red-700 ">
                                              <a href="https://github.com/themesberg/flowbite" className=" ">{des}</a>
                                            </li>
                                          )
                                          })
                                      }
